HealthCheck.setup do |config|
  ...
  # config.full_checks = %w[database migrations custom email cache redis resque-redis sidekiq-redis
  #                         s3]
  ...
  # (uses IPAddr.include? method to check)
  # Defaults to blank which allows any IP
  # config.origin_ip_whitelist = %w(123.123.123.123 10.11.12.0/24 2400:cb00::/32)

  # Use ActionDispatch::Request's remote_ip method when behind a proxy to pick up the real remote IP
  # for origin_ip_whitelist check
  # Otherwise uses Rack::Request's ip method (the default, and always used by Middleware), which is
  #  more susceptable to spoofing
